// Code built off of Will Day's Minimul Stub Plugin
// Credit to Will Day for the All-Powerful MetaMod
...
//this file contains interaction with MetaMod
...
static META_FUNCTIONS gMetaFunctionTable = {
	...
	NULL,//GetNewDLLFunctions,	// pfnGetNewDLLFunctions		HL SDK2; called before game DLL
	NULL,//GetNewDLLFunctions_Post,// pfnGetNewDLLFunctions_Post		META; called after game DLL
	GetEngineFunctions,	// pfnGetEngineFunctions		META; called before HL engine
	NULL,//GetEngineFunctions_Post	// pfnGetEngineFunctions_Post		META; called after HL engine
};
...
plugin_info_t Plugin_info = {
	...
	"WEAPTFC_MM",					// name
	WEAPTFC_VERSION,					// version
	__DATE__,					// date
	"CyberMind - Kevin Masterson",			// author
	"http://logd.sf.net/weaptfc/",			// url
	"WTFC",						// logtag
...
{
	Plugin_info.ifvers = ifvers;	//use whatever ifver MetaMod is using so we won't differ
...
{
	if(now)now=now;	// to satisfy gcc -Wunused
	if(!pMGlobals) {
...
C_DLLEXPORT int Meta_Detach(PLUG_LOADTIME now, PL_UNLOAD_REASON reason) {
	if(now && reason)now=now;	// to satisfy gcc -Wunused
